...
void draw() {
	...
	fill(0, 2, 0);
    text("Gráfico interativo do resultados do último enfrentamento entre Seleções em um período de 4 anos.", numSelecoes*3, 8+9);
	textSize(12);
	text("Times em cada linha ordenados segundo ranking de eficiência. Código de cores: Verde=Vitória || Cinza=Empate || Vermelho Derrota.", numSelecoes*3, 8+25);
  ...
  rect(margemEsq,margemSup,numSelecoes*2*(sizeMark+afast),numSelecoes*2*(sizeMark+afast)); //retangulo de teste da area sensivel
	...
	fill(240);
	int Xsensivel = 4;
	rect(Xsensivel, margemSup, margemEsq-Xsensivel, numSelecoes*2*(sizeMark+afast));
	
	if (mouseX>Xsensivel && mouseX<Xsensivel+margemEsq){
		if(mouseY>margemSup && mouseY<margemSup+numSelecoes*2*(sizeMark+afast)){
