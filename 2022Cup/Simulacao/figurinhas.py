...
# É necessário instalar o AggDraw: conda install -c conda-forge aggdraw
import aggdraw
...
def figurinha(sel1, sel2, cod1, cod2, alfa1, beta1, alfa2, beta2):
    ...
    f1 = ImageFont.truetype("arial.ttf", 23)
    #draw.text([175, 28], "Placar mais provável:", font=f1, fill=(0, 0, 0, 255))
    draw.text([150, 20], "Placar mais provável:", font=f1, fill=(255, 255, 255, 255))
    ...
    draw.text([x, y], str(g1mp) + " x " + str(g2mp), font=f2, fill=(255, 255, 255, 255))
    ...
    draw.text([x, y], "Probabilidade do placar: " + prob + "%", font=f1, fill=(255, 255, 255, 255))
    ...
    draw.text([x, y], sel1, font=f4, fill=(255, 255, 255, 255))
    sz = draw.textsize(sel2, spacing=0, font=f4)
    x = 260
    y = 580
    #draw.text([x, y], sel2, font=f4, fill=(0, 0, 0, 255))
    draw.text([x, y], sel2, font=f4, fill=(255, 255, 255, 255))
    ...
    im.save("disputas/" + cod1 + "X" + cod2 + ".png")
    # Depois de rodar, tem que copiar as figurinhas geradas para a pasta correta dentro do site

def figurinhaADefinir():
    ...
    f6 = ImageFont.truetype("arial.ttf", 10)
    f20 = ImageFont.truetype("arial.ttf", 40)
    
    draw.text([120, 260], 'JOGO A SER', font=f20, fill=(0, 0, 0, 0))
    draw.text([150, 300], 'DEFINIDO', font=f20, fill=(0, 0, 0, 0))

    draw.text([5, 638], 'FGV-EMAp - Escola de Matemática Aplicada', font=f6, fill=(0, 0, 0, 208))
    draw.text([362, 638], 'Projeto Esporte em Números', font=f6, fill=(0, 0, 0, 208))
    im.save("disputas/ADEFINIR.png")


#alfas e betas bem chutados pra dar um placar absurdo para o Brasil
#figurinha("BRASIL", "ESPANHA", "BRA" , "ESP", 4.7533, 1.5214, 2.3867, 1.0357)
#figurinha("BRASIL", "ESPANHA", "BRA" , "ESP", 3.7533, 2.0214, 1.9867, 1.5357)
#figurinhaADefinir()

# Jogos das Oitavas de final 2022 - colocados "na força bruta" para fazer as figurinhas
#figurinha("HOLANDA","ESTADOS UNIDOS","NED","USA",2.8680411,2.5012301,1.5218207,2.4897686)
#figurinha("ARGENTINA","AUSTRÁLIA","ARG","AUS",2.7509797,3.363779,1.6427147,1.7718426)
#figurinha("JAPÃO","CROÁCIA","JPN","CRO",1.9638616,2.2343138,2.4014045,2.167201)
#figurinha("BRASIL","COREIA DO SUL","BRA","KOR",2.9235103,5.9757602,1.687641,1.808989)
#figurinha("FRANÇA","POLÔNIA","FRA","POL",3.0713668,2.1381048,2.0494314,1.6911287)
#figurinha("INGLATERRA","SENEGAL","ENG","SEN",3.3260984,2.4228409,1.8677611,1.8309102)
#figurinha("MARROCOS","ESPANHA","MAR","ESP",2.2470444,3.1440148,3.4440189,2.463000)
#figurinha("PORTUGAL","SUÍÇA","POR","SUI",3.0763805,2.2281934,2.4066802,2.127600)

# Jogos das Quartas de final 2022 - colocados "na força bruta" para fazer as figurinhas
#figurinha("CROÁCIA","BRASIL","CRO","BRA",2.45731949,2.19158226,3.19804012,4.98224850)
#figurinha("HOLANDA","ARGENTINA","NED","ARG",3.08230787,2.40978366,2.82697973,3.12850420)
#figurinha("MARROCOS","PORTUGAL","MAR","POR",2.11675444,3.85233450,3.61417948,2.27053452)
#figurinha("INGLATERRA","FRANÇA","ENG","FRA",3.45201444,2.67420118,3.26905781,2.19086560)

# Jogos das Semi-finais 2022 - colocados "na força bruta" para fazer as figurinhas
#figurinha("ARGENTINA","CROÁCIA","ARG","CRO",2.91630898,2.71055806,2.55152985,2.28241227)
#figurinha("FRANÇA","MARROCOS","FRA","MAR",3.38807948,2.30452322,2.13334315,4.64893045)

# Jogos das Finais 2022 - colocados "na força bruta" para fazer as figurinhas
figurinha("ARGENTINA","FRANÇA","ARG","FRA",3.19698991,3.06556153,3.53997669,2.48108114)
figurinha("CROÁCIA","MARROCOS","CRO","MAR",2.42465252,2.03143402,2.00172365,3.73177491)
